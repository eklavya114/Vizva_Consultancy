...
/**
 * CTS Application Root
 * Orchestrates high-level routing, modal states, and USA-localized 
 * ticket creation. 
 */
...
import { Plus, X, Database, Phone, Zap } from 'lucide-react';
...
const AppContent: React.FC = () => {
  ...
  /** View ticket details with a specific context */
  const handleTicketClick = (id: string) => {
    setSelectedTicketId(id);
    setActiveTab('ticket-detail');
  };

  /** Validates and commits a new ticket to the global store */
  const handleCreateTicket = (e: React.FormEvent) => {
    e.preventDefault();
    const phoneClean = newTicketForm.phone.replace(/\D/g, '');
    
    // USA Localization: 10-digit validation
    if (phoneClean.length !== 10) {
      alert("Localization Error: Please enter a valid 10-digit US mobile number.");
      return;
    }
  ...
  return (
    <Layout activeTab={activeTab} setActiveTab={setActiveTab}>
      {/* Dynamic Tab Rendering based on RBAC & State */}
      {activeTab === 'dashboard' && (
        <Dashboard 
      ...
      {/* Creation Modal: Implements the "Initialize Engagement" flow */}
      {showCreateModal && (
